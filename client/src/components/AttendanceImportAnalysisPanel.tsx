...
import { BarChart3, CircleAlert, Clock3, FileDown, FileText, Save, UsersRound } from "lucide-react";
...
export function AttendanceImportAnalysisPanel({ activeBranchId, draft, onSettingsChange }: AttendanceImportAnalysisPanelProps) {
  ...
  const handlePdfExport = async () => {
    ...
    try { const result = await exportAttendanceImportAnalysisPdf(reportRef.current, draft); toast.success(`تم تجهيز ${result.filename}.`); } catch { toast.error("تعذر تجهيز ملف PDF. حاول مرة أخرى."); } finally { setExportingPdf(false); }
  };
  ...
  return <Card className="border-[#cce3d8] bg-[#f8fcfa]"><CardContent className="p-5">
    ...
    <div className="mt-4 flex flex-wrap gap-2 border-t border-[#d7e6df] pt-4"><Button size="sm" type="button" variant="outline" disabled={!activeBranchId || saveDefault.isPending} onClick={handleSaveDefault} className="border-[#b9d8ca] text-[#0f766e] hover:bg-[#eaf4ef] hover:text-[#0f766e]"><Save className="ml-1 h-3.5 w-3.5" />{saveDefault.isPending ? "جارٍ الحفظ…" : "حفظ كسياسة افتراضية"}</Button><Button size="sm" type="button" variant="outline" onClick={() => { exportAttendanceImportAnalysisExcel(draft, settings, analysis); toast.success("تم تجهيز تقرير Excel للإدارة."); }} className="border-[#b9d8ca] text-[#0f766e] hover:bg-[#eaf4ef] hover:text-[#0f766e]"><FileDown className="ml-1 h-3.5 w-3.5" />تصدير Excel</Button><Button size="sm" type="button" variant="outline" disabled={exportingPdf} onClick={handlePdfExport} className="border-[#17344a] text-[#17344a] hover:bg-slate-50 hover:text-[#17344a]"><FileText className="ml-1 h-3.5 w-3.5" />{exportingPdf ? "جارٍ تجهيز PDF…" : "تصدير PDF"}</Button></div>
  </CardContent></Card>;
}
