...
export default function DeliveryCenter() {
  ...
  const [zoneForm, setZoneForm] = useState({ name: "", code: "", description: "", slaMinutes: 60, slaWarningMinutes: 10 });
  ...
  const saveZone = trpc.delivery.saveZone.useMutation({ onSuccess: () => { toast.success("تم حفظ منطقة التوصيل ونافذة التحذير."); setZoneForm({ name: "", code: "", description: "", slaMinutes: 60, slaWarningMinutes: 10 }); utils.delivery.zones.invalidate(); utils.delivery.slaAlerts.invalidate(); }, onError: error => toast.error(error.message) });
  ...
  return <div dir="rtl" className="space-y-6">
    ...
    {manager ? <>
      ...
      <section className={`rounded-3xl border p-5 ${alerts.data?.length ? "border-amber-200 bg-amber-50" : "border-emerald-100 bg-emerald-50"}`}><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><AlertTriangle className={`h-5 w-5 ${alerts.data?.length ? "text-amber-700" : "text-emerald-700"}`} /><h3 className="font-extrabold">تنبيهات اتفاقية مستوى الخدمة</h3></div><span className="text-xs text-slate-500">تُحدّث تلقائياً أثناء فتح الصفحة، ولا تمثل تنبيهاً خلفياً مستقلاً.</span></div>{alerts.data?.length ? <div className="mt-3 grid gap-2 md:grid-cols-2">{alerts.data.map(alert => <div key={alert.order.id} className={`rounded-2xl p-3 text-sm ${alert.state === "breached" ? "bg-rose-100 text-rose-900" : "bg-amber-100 text-amber-900"}`}><b>{alert.order.orderCode} · {alert.agentName ?? "لم يُعيّن مندوب"}</b><p className="mt-1">{alert.zoneName ?? "بدون منطقة"} · {alert.minutesRemaining < 0 ? `تم التجاوز بـ ${Math.abs(alert.minutesRemaining)} دقيقة` : `متبقي ${alert.minutesRemaining} دقيقة`}</p><p className="mt-1 text-xs opacity-80">نافذة التحذير: {alert.warningMinutes} دقائق</p></div>)}</div> : <p className="mt-2 text-sm text-emerald-800">لا توجد رحلة قريبة من تجاوز SLA أو متجاوزة حالياً.</p>}</section>
