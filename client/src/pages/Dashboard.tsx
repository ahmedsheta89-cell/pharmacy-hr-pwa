...
function HeroSignal({ icon: Icon, label, note }: { icon: LucideIcon; label: string; note: string }) {
  return <div className="flex min-w-32 items-center gap-2 rounded-2xl border border-white/10 bg-white/[.055] px-3 py-2.5 backdrop-blur-sm"><span className="grid h-8 w-8 place-items-center rounded-xl bg-[#9ee4c9]/15 text-[#a7ebcf]"><Icon className="h-4 w-4" /></span><span><span className="block text-[11px] font-extrabold text-white">{label}</span><span className="mt-0.5 block text-[10px] text-slate-300">{note}</span></span></div>;
}
...
export default function Dashboard() {
  ...
  return <div className="space-y-6" dir="rtl">
    <section className="relative overflow-hidden rounded-[1.75rem] bg-[#17344a] px-6 py-7 text-white shadow-[0_30px_60px_-36px_rgba(23,52,74,.72)] md:px-8 md:py-8"><div className="relative z-10"><div className="flex flex-col justify-between gap-7 lg:flex-row lg:items-end"><div className="max-w-2xl"><div className="flex flex-wrap items-center gap-2"><Badge className="border-0 bg-white/10 px-3 py-1 text-[#a7ebcf] hover:bg-white/10">{config.badge}</Badge><span className="inline-flex items-center gap-1 rounded-full border border-[#9ee4c9]/20 bg-[#9ee4c9]/10 px-2.5 py-1 text-[10px] font-bold text-[#c7f6df]"><Pill className="h-3 w-3" />غرفة عمليات الصيدلية</span></div><h2 className="mt-4 text-2xl font-extrabold tracking-tight md:text-3xl">{config.title}</h2><p className="mt-3 max-w-xl text-sm leading-7 text-slate-300">{config.description}</p></div><div className="flex flex-wrap gap-3"><Button variant="outline" onClick={exportDashboardReport} disabled={overviewQuery.isLoading} className="h-11 rounded-xl border-white/20 bg-white/5 px-5 font-bold text-white hover:bg-white/15 hover:text-white"><FileDown className="ml-2 h-4 w-4" />تصدير التقرير</Button><Button onClick={() => setLocation(config.primaryPath)} className="h-11 rounded-xl bg-[#9ee4c9] px-5 font-bold text-[#123c35] hover:bg-[#c1efd9]"><Plus className="ml-2 h-4 w-4" />{config.primaryAction}</Button><Button variant="outline" onClick={() => setLocation(config.secondaryPath)} className="h-11 rounded-xl border-white/20 bg-white/5 px-5 font-bold text-white hover:bg-white/15 hover:text-white">{config.secondaryAction}</Button></div></div><div className="mt-6 flex flex-wrap gap-2"><HeroSignal icon={Building2} label="الفروع" note="جاهزية التشغيل" /><HeroSignal icon={UsersRound} label="الفريق" note="حضور وورديات" /><HeroSignal icon={WalletCards} label="الرواتب" note="قبل الاعتماد" /><HeroSignal icon={HeartPulse} label="الامتثال" note="قرارات المالك" /></div></div><Pill className="pointer-events-none absolute -bottom-14 -left-10 h-52 w-52 rotate-[-22deg] text-white/[.035]" strokeWidth={1} /><div className="pointer-events-none absolute -ml-16 -mt-44 h-72 w-72 rounded-full bg-[#0f766e]/30 blur-3xl" /></section>
