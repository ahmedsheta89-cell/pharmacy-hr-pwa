...
export const kpiDefinitions = mysqlTable("kpiDefinitions", {
  ...
  ownerEmployeeId: int("ownerEmployeeId").references(() => employees.id, { onDelete: "set null" }),
  name: varchar("name", { length: 160 }).notNull(),
  ...
  direction: mysqlEnum("direction", ["higher_better", "lower_better"]).default("higher_better").notNull(),
...
export const deliveryZones = mysqlTable("deliveryZones", {
  ...
  slaWarningMinutes: int("slaWarningMinutes").default(10).notNull(),
  isActive: mysqlEnum("isActive", ["yes", "no"]).default("yes").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [
